...
BASE_URL = "http://localhost:8080"  # Основной CRUD сервис
AVAILABILITY_URL = "http://localhost:8081"  # Микросервис доступности
fake = Faker()
...
async def check_availability(session, flight_id):
    try:
        async with session.get(f"{AVAILABILITY_URL}/api/availability?flightId={flight_id}") as response:
            if response.status == 200:
                data = await response.json()
                print(f"Доступно мест на рейс {flight_id}: {data['availableSeats']}")
    except Exception as e:
        print(f"Ошибка проверки доступности: {str(e)}")
...
async def generate_data(passengers_count, flights_count, bookings_count):
    ...
    # Проверяем доступность и создаем бронирования
    async with aiohttp.ClientSession() as session:
        # Проверка доступности
        await asyncio.gather(*[check_availability(session, f['id']) for f in flights])
